...
from gws_core import (CondaShellProxy, ConfigParams, File, InputSpec, ConfigSpecs,
...
class SoBlast(Task):
    ...
    def run(self, params: ConfigParams, inputs: TaskInputs) -> TaskOutputs:
        ...
        file_downloader = TaskFileDownloader(
            SoBlast.get_brick_name(), self.message_dispatcher)

        # download the db and retrieve the path of the downloaded file
        zebra_zipped_db = file_downloader.download_file_if_missing(
            db_url, db_file_name)
        ...
        result = shell_proxy.run(
            f"gunzip -c {zebra_zipped_db} > {zebra_db}", shell_mode=True)
        if result != 0:
        ...
        result = shell_proxy.run(
            f"makeblastdb -in {zebra_db} -dbtype prot -out {zebra_db}", shell_mode=True)
        if result != 0:
        ...
        result = shell_proxy.run(
            f"gunzip -c {file.path} > {input_file_unzipped}", shell_mode=True)
        if result != 0:
        ...
        if head > 0:
            ...
            result = shell_proxy.run(
                f"head -n {head} {input_file_unzipped} > {sub_file}", shell_mode=True)
